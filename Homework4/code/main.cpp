...
cv::Point2f recursive_bezier(const std::vector<cv::Point2f>& control_points, float t)
{
    ...
    std::vector<cv::Point2f> lerped_control_points;
    for (int i = 0; i < control_points.size() - 1; i++)
    {
        auto lerped_point = (1 - t) * control_points[i] + t * control_points[i + 1];
        lerped_control_points.push_back(lerped_point);
    }

    return recursive_bezier(lerped_control_points, t);
}
